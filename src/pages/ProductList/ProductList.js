...
function ProductList() {
  ...
  const getProduct = () => {
    fetch(`http://10.58.6.47:8000/products?category=1&sub_category=2&id=3`)
      .then((res) => res.json())
  ...
  // react-rouer-dom query parameter
  // advanced router query parameter
  // backend query front query
  ...
  return (
    ...
    <HeaderContainer>
      ...
      <HeaderUl>
        <HeaderLi>전 상품</HeaderLi><HeaderLiSector>/</HeaderLiSector>
        <HeaderLi>반팔</HeaderLi><HeaderLiSector>/</HeaderLiSector>
      </HeaderUl>
    </HeaderContainer>
    <Section>
      <SectionAside>
        <SectionTitleWraper>
          {Object.keys(productList).map((title)=>{
            return(
            <SectionTitleList>{title}</SectionTitleList>
          )})}
        </SectionTitleWraper>
          {Object.entries(productList).map(([title,subtitle])=>{
            return(
              ...
              <FilterTitle FilterTitle onClick={()=>setCheck(!check)}>{title}</FilterTitle>
              </SectionFilter>
          ...
          <SectionListWraper>
            <SectionFilter>
              <FilterCheckbox type="checkbox" checked={check} />
              <FilterTitle FilterTitle onClick={()=>setCheck(!check)}>컬러 & 패턴</FilterTitle>
              </SectionFilter>
                <FilterListContainer checked={check}>
                  <FilterListWrap>
                    <FilterList>
                      <ColorPickerWraper>
                        {Object.entries(COLOR).map(([name, property])=>{
                          ...
                          return(
                            <ColorPicker>
                              <ColorPickerBtn style={{backgroundColor:`${code}`}} />
                              {name} 
                            </ColorPicker>
                            )
                          }
...
const ProductListContainer = styled.header`
  padding: 0 8vw;
`
